...
// From https://www.reddit.com/r/Unity3D/comments/14efmja/prefab_thumbnail_saver_editor_script/
public static class PrefabThumbnailSaver
{
    ...
    private static void ExportPrefabThumbnail()
    {
        ...
        if (selectedObject != null && PrefabUtility.GetPrefabAssetType(selectedObject) != PrefabAssetType.MissingAsset)
        {
            ...
            Texture2D thumbnailTexture = GenerateTransparentThumbnail(prefab);
        ...
        if (thumbnailTexture != null)
        {
            ...
            for (int i = 0; i < pixels.Length; i++)
            {
                ...
                if (pixels[i] == backgroundColor)
                {
                    ...
                    pixels[i].a = 0; // If this pixel is exactly the background color, make it transparent
